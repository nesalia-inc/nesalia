...
## Rule 8: Use "You" and learner-oriented perspective

When introducing concepts, write from the learner's perspective. Use "you" to make the content more engaging and less directive. The learner is the protagonist, not the reader observing from afar.

```
❌ BAD:
We create variables by assigning a value to a name.

✅ GOOD:
To create a variable, you assign a value to a name using the equals sign.
```

Exception: When describing what "we" do as a convention or common practice in the industry, "We" is appropriate.
...
## Rule 9: Explain Technical Terms

Never use a technical term without explaining it. If you introduce jargon, immediately follow with a plain-language explanation or definition.
...
✅ GOOD:
Strings support concatenation with the + operator. Concatenation means joining two or more strings end to end.
```
...
✅ GOOD:
Python has several core data types. "Core" means these types are available by default in Python without needing to import any external library.
```

**Key terms to always explain:**
- concatenation (joining strings together)
- built-in (types available by default - use "core" instead)
- iteration (repeating a process)
- mutation (changing a value after creation)
- hashable (can be used as dictionary key)

## Rule 10: Paragraph After Every Code Block

Every code block must be followed by an explanatory paragraph. The learner should never finish a code block without receiving context about what the code does.
...
print(name)
```
This code creates a variable called `name` and assigns it the string "Alice". The print statement outputs the value to the console.
```

**The paragraph should:**
- Explain what the code does
- Highlight key parts relevant to the current topic
- Connect to the surrounding explanation

## Rule 11: Cross-Reference Related Sections

Always link to other relevant sections in the course. This helps learners understand how concepts connect and encourages them to explore further.

```
✅ GOOD:
The asterisk (*) performs repetition when used with a string and an integer.
...
```
✅ GOOD:
Booleans are fundamental to programming because every decision in a program depends on a boolean value.
...
- When a concept uses operators (link to operators section)
- When a concept is used in control flow (link to relevant control flow section)
- When a data type has related methods (link to the data type section)
- When a concept builds on earlier learning (link to prerequisite)

## Rule 12: Expand Important or Complex Sections

Some concepts are particularly important or prone to confusion. These sections should:
- Slow down the pace
- Provide multiple examples
- Explain the "why" behind the concept
- Show real-world use cases

**Sections that typically need expansion:**
- `None` - Explain why it exists, when to use it, `is None` vs `== None`
- Type conversion - Cover all directions and common patterns
- Booleans - Explain practical uses and connection to boolean algebra
- Variable scope - Common source of bugs

```
✅ GOOD for None:
## None Type

Sometimes, a variable has no value at all. Not zero, not an empty string, but literally the absence of a value...
...
### Checking for None
To check if a variable is None, you use the is operator rather than...
```
